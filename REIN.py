...
class REM(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.angles)):
            ...
            grid = F.affine_grid(aff, torch.Size(x.size()), align_corners=True).type(
                x.type()
            )
            ...
            aff[:, 1, 1] = torch.cos(self.angles[i])
            grid = F.affine_grid(
                aff, torch.Size(im1_init_size), align_corners=True
            ).type(x.type())
        ...
        equ_features = torch.max(equ_features, dim=-1, keepdim=False)[
            0
        ...
        B, C, H, W = x.size()
        grid = F.affine_grid(
            aff, torch.Size((B, C, H // 4, W // 4)), align_corners=True
        ).type(
            x.type()
        )  # ,align_corners=True)
